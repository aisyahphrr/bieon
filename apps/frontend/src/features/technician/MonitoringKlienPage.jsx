...
export function MonitoringKlienPage({ clients = [] }) {
    ...
    const getStatusLabel = (status) => {
        switch (status) {
            case 'Semua': return t('complaint.status_all', 'Semua');
            case 'Online': return t('tech_dashboard.table.status_online', 'Online');
            case 'Warning': return t('tech_dashboard.table.status_warning', 'Warning');
            case 'Offline': return t('tech_dashboard.table.status_offline', 'Offline');
        ...
        <div className="w-full pb-10 animate-in fade-in duration-500">
            ...
            <div className="mb-8">
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <h1 className="text-2xl font-black text-gray-900 tracking-tight">{t('tech_dashboard.monitoring.title', 'Monitoring Kesehatan Sistem')}</h1>
                </div>
                <p className="text-gray-500 text-sm">{t('tech_dashboard.monitoring.subtitle', 'Pantau status konektivitas Hub dan Perangkat seluruh pelanggan secara real-time.')}</p>
            </div>

            {/* Quick Stats Grid */}
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                <div className="bg-white rounded-[2rem] p-5 shadow-sm border border-gray-100 transition-all hover:shadow-md">
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">{t('tech_dashboard.metrics.total_clients', 'Total Klien')}</p>
                ...
                <div className="bg-emerald-50/50 rounded-[2rem] p-5 shadow-sm border border-emerald-100 transition-all hover:shadow-md">
                    <p className="text-[10px] font-bold text-emerald-600/60 uppercase tracking-widest mb-1">{t('tech_dashboard.monitoring.stats.system_normal', 'Sistem Normal')}</p>
                    <div className="flex items-end justify-between">
                ...
                <div className="bg-rose-50/50 rounded-[2rem] p-5 shadow-sm border border-rose-100 transition-all hover:shadow-md">
                    <p className="text-[10px] font-bold text-rose-600/60 uppercase tracking-widest mb-1">{t('tech_dashboard.monitoring.stats.system_down', 'System Down')}</p>
                    <div className="flex items-end justify-between">
                ...
                <div className="bg-amber-50/50 rounded-[2rem] p-5 shadow-sm border border-amber-100 transition-all hover:shadow-md">
                    <p className="text-[10px] font-bold text-amber-600/60 uppercase tracking-widest mb-1">{t('history.status.warning', 'Peringatan')}</p>
                    <div className="flex items-end justify-between">
            ...
            <div className="bg-white/80 backdrop-blur-md rounded-3xl p-4 shadow-sm border border-gray-100 mb-6 flex flex-col md:flex-row gap-4 items-center justify-between sticky top-4 z-20">
                <div className="relative w-full md:w-96 group">
                    ...
                    <input
                        type="text"
                        placeholder={t('tech_dashboard.monitoring.search_placeholder', 'Cari ID, Nama, atau Lokasi...')}
                        value={searchQuery}
                ...
                <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto pb-2 md:pb-0 scrollbar-hide">
                    {['Semua', 'Online', 'Warning', 'Offline'].map((status) => (
                        <button
                            ...
                            className={`px-6 py-2.5 rounded-xl text-xs font-bold transition-all whitespace-nowrap ${statusFilter === status
            ...
            <div className="bg-white rounded-[2.5rem] shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
                            ...
                            <tr className="bg-gray-50/50 border-b border-gray-100">
                                <th onClick={() => handleSort('nama')} className="px-8 py-5 text-[11px] font-black text-gray-400 uppercase tracking-widest cursor-pointer group">
                                    <div className="flex items-center gap-2">
                                        {t('history.columns.customer', 'Data Pelanggan')}
                                        {getSortIcon('nama')}
                                    </div>
                                </th>
                                <th className="px-6 py-5 text-[11px] font-black text-gray-400 uppercase tracking-widest text-center">{t('tech_dashboard.monitoring.table.infrastructure', 'Infrastruktur')}</th>
                                <th onClick={() => handleSort('status')} className="px-6 py-5 text-[11px] font-black text-gray-400 uppercase tracking-widest text-center cursor-pointer">
                                    <div className="flex items-center justify-center gap-2">
                                        {t('tech_dashboard.monitoring.table.system_health', 'Kesehatan Sistem')}
                                        {getSortIcon('status')}
                                    </div>
                                </th>
                                <th className="px-6 py-5 text-[11px] font-black text-gray-400 uppercase tracking-widest">{t('tech_dashboard.monitoring.table.status_detail', 'Detail Status')}</th>
                                <th className="px-8 py-5 text-right text-[11px] font-black text-gray-400 uppercase tracking-widest">{t('history.columns.action', 'Aksi')}</th>
                            </tr>
                        </thead>
                                    ...
                                    <tr key={client.id} className="hover:bg-emerald-50/30 transition-colors group">
                                        <td className="px-8 py-6">
                                            <div className="flex items-center gap-4">
                                                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-sm border ${client.status === 'online' ? 'bg-eco/10 text-eco border-eco/20' :
                                        ...
                                        <td className="px-6 py-6">
                                            <div className="flex justify-center">
                                                <div className={`flex items-center gap-2 px-4 py-2 rounded-2xl border font-black text-[11px] uppercase tracking-wider ${client.status === 'online' ? 'bg-eco/10 text-eco border-eco/20 shadow-sm shadow-eco/10' :
                                                    ...
                                                    <div className={`w-2 h-2 rounded-full ${client.status === 'online' ? 'bg-eco' :
                                                            client.status === 'warning' ? 'bg-amber-500' :
                                                                'bg-rose-500'
                                                        }`} />
                                                    {client.status === 'online' ? t('tech_dashboard.monitoring.status.normal', 'System Normal') :
                                                        client.status === 'warning' ? t('tech_dashboard.monitoring.status.warning', 'Warning Alert') :
                                                            t('tech_dashboard.monitoring.status.down', 'System Down')}
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-6">
                                            <div className="max-w-[200px]">
                                                <p className="text-xs font-bold text-gray-700 mb-1 truncate">{localizedSystemStatus}</p>
                ...
                {filteredClients.length === 0 && (
                    <div className="py-20 text-center">
                        ...
                        <h4 className="text-lg font-bold text-gray-900 mb-1">{t('tech_dashboard.monitoring.no_results', 'Hasil Tidak Ditemukan')}</h4>
                        <p className="text-gray-500 text-sm">{t('tech_dashboard.monitoring.no_results_desc', 'Coba gunakan kata kunci lain atau filter status yang berbeda.')}</p>
                    </div>
                )}

                <div className="px-8 py-5 bg-gray-50/50 border-t border-gray-100 flex items-center justify-between">
                    <p className="text-xs font-bold text-gray-400">
                        {t('tech_dashboard.monitoring.showing_clients', 'Menampilkan {{showing}} dari {{total}} pelanggan ditangani', {
                    ...
                    <div className="flex items-center gap-2">
                        <div className="flex items-center gap-1 bg-white border border-gray-200 px-3 py-1.5 rounded-xl">
                            ...
                            <span className="text-[10px] font-black text-gray-600 uppercase">{t('tech_dashboard.monitoring.live_sync', 'Live Sync Active')}</span>
                        </div>
                    </div>
                </div>
            </div>
