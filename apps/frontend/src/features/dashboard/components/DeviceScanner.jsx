...
const LOCAL_DEVICE_IDENTIFIER = {
  ...
  "SNZB-04": {
    ...
    icon: <Activity className="w-6 h-6 text-sense" />
  }
};
...
const DeviceScanner = ({ onScanSuccess, onCancel }) => {
    ...
    try {
      ...
      if (data.isValid) {
        ...
        setDetectedDevice({ 
          ...
          icon: <Activity className="w-6 h-6 text-eco" />,
          rawCode: code 
        });
        setStep("detected");
      } else {
  ...
  return (
    <div className="bg-white rounded-3xl shadow-2xl overflow-hidden max-w-lg w-full border border-gray-100">
      {/* Header */}
      <div className="bg-gradient-to-r from-gray-900 to-eco-900 p-6 text-white flex items-center justify-between">
        <div className="flex items-center gap-3">
          <QrCode className="w-6 h-6 text-eco" />
          <div>
            <h3 className="text-xl font-bold">Smart Pairing</h3>
            <p className="text-[10px] text-eco-50 uppercase tracking-widest font-black">Identify & Connect</p>
          </div>
        </div>
      ...
      <div className="p-8">
        ...
        {(step === "idle" || step === "scanning") && (
          <div className="space-y-8">
            <div id={qrRegionId} className={`w-full aspect-square bg-gray-50 rounded-3xl border-4 border-dashed flex items-center justify-center ${step === "scanning" ? "border-eco bg-black" : "border-gray-200"}`}>
              {step === "idle" && (
                <div className="text-center p-6">
                  <div className="w-20 h-20 bg-eco/5 rounded-3xl flex items-center justify-center mx-auto mb-6">
                    <Camera className="w-10 h-10 text-eco" />
                  </div>
                  <p className="text-gray-600 font-bold mb-6">Scan QR untuk mengenali perangkat</p>
                  <button onClick={startScanner} className="px-10 py-4 bg-eco hover:bg-eco/90 text-white rounded-2xl font-black shadow-xl transition-all active:scale-95">
                    Buka Scanner
                  </button>
                </div>
              )}
        ...
        {step === "detected" && detectedDevice && (
          <div className="animate-in fade-in zoom-in duration-300">
            <div className="bg-eco/5 rounded-[2.5rem] p-8 text-center border-2 border-eco/10">
              ...
              <p className="text-eco font-bold mb-6">Model: {detectedDevice.model}</p>
              ...
              <div className="bg-white/60 rounded-2xl p-6 text-left border border-white mb-8">
                ...
                <div className="flex flex-wrap gap-2">
                  {detectedDevice.params.map((p, i) => (
                    <span key={i} className="px-3 py-1 bg-eco/10 text-eco text-xs font-bold rounded-lg">{p}</span>
                  ))}
                </div>
              </div>
              ...
              <div className="flex flex-col gap-3">
                <button onClick={handleStartPairing} className="w-full py-4 bg-eco hover:bg-eco/90 text-white rounded-2xl font-black shadow-xl shadow-eco/20 flex items-center justify-center gap-2 group">
        ...
        {step === "pairing" && (
          <div className="py-12 text-center animate-in fade-in duration-300">
            <div className="relative w-32 h-32 mx-auto mb-8">
              <div className="absolute inset-0 border-4 border-eco/10 rounded-full"></div>
              <div className="absolute inset-0 border-4 border-eco rounded-full border-t-transparent animate-spin"></div>
              <Radio className="absolute inset-0 m-auto w-10 h-10 text-eco animate-pulse" />
            </div>
        ...
        {step === "success" && (
          <div className="py-12 text-center animate-in zoom-in duration-500">
            <div className="w-24 h-24 bg-eco/10 rounded-full flex items-center justify-center mx-auto mb-6">
              <Check className="w-12 h-12 text-eco" />
            </div>
            <h4 className="text-2xl font-black text-gray-900 mb-2">Terhubung!</h4>
            <p className="text-eco font-bold">{detectedDevice?.name} berhasil diverifikasi di jaringan Zigbee.</p>
          </div>
        )}
      </div>
    </div>
